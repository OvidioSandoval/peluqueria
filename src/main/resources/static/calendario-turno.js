...
new Vue({
    ...
    template: '<div class="glass-container"><div><h1 class="page-title">Calendario de Turnos</h1><button @click="window.history.back()" class="btn"><i class="fas fa-arrow-left"></i> Volver</button><main style="padding: 20px;"><div class="calendar-controls" style="display: flex; justify-content: center; align-items: center; margin-bottom: 20px;"><div class="month-navigation" style="display: flex; align-items: center; gap: 15px;"><button @click="mesAnterior()" class="btn"><i class="fas fa-chevron-left"></i></button><h3 style="margin: 0; color: #5d4037;">{{ meses[mesActual] }} {{ añoActual }}</h3><button @click="mesSiguiente()" class="btn"><i class="fas fa-chevron-right"></i></button></div></div><div v-if="cargando" class="loading" style="text-align: center; padding: 40px;"><i class="fas fa-spinner fa-spin"></i> Cargando calendario...</div><div v-else class="calendar-grid" class="btn"><div v-for="dia in diasSemana" :key="dia" class="calendar-header" class="btn">{{ dia }}</div><div v-for="(dia, index) in diasDelMes" :key="index" class="calendar-day" :class="{ today: dia && dia.esHoy, empty: !dia }" class="btn"><div v-if="dia" class="day-content"><div class="day-number" :class="btn">{{ dia.numero }}</div><div class="turnos-container" style="max-height: 90px; overflow-y: auto;"><div v-for="turno in dia.turnos" :key="turno.id" class="turno-item" @click="verDetalleTurno(turno)" class="btn" :style="{ borderLeftColor: getColorEstado(turno.estado) }"><div style="font-weight: bold; color: #333;">{{ formatearHora(turno.hora) }}</div><div style="color: #666; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ turno.cliente ? turno.cliente.nombreCompleto : "Sin cliente" }}</div><div style="color: #888; font-size: 10px;">{{ turno.servicio ? turno.servicio.nombre : "Sin servicio" }}</div></div></div></div></div></div><div v-if="mostrarDetalle && turnoSeleccionado" class="turno-detail-modal" class="btn"><div class="modal-content" class="btn"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h3 style="margin: 0; color: #5d4037;"><i class="fas fa-calendar-check"></i> Detalle del Turno</h3><button @click="cerrarDetalle()" class="btn" class="btn"><i class="fas fa-times"></i></button></div><div class="turno-details" style="display: grid; gap: 15px;"><div class="detail-row" class="btn"><i class="fas fa-user" style="color: #5d4037; width: 20px; margin-right: 10px;"></i><div><strong>Cliente:</strong> {{ turnoSeleccionado.cliente ? turnoSeleccionado.cliente.nombreCompleto : "Sin cliente" }}</div></div><div class="detail-row" class="btn"><i class="fas fa-cut" style="color: #5d4037; width: 20px; margin-right: 10px;"></i><div><strong>Servicio:</strong> {{ turnoSeleccionado.servicio ? turnoSeleccionado.servicio.nombre : "Sin servicio" }}</div></div><div class="detail-row" class="btn"><i class="fas fa-user-tie" style="color: #5d4037; width: 20px; margin-right: 10px;"></i><div><strong>Colaborador:</strong> {{ turnoSeleccionado.empleado ? turnoSeleccionado.empleado.nombreCompleto : "Sin empleado" }}</div></div><div class="detail-row" class="btn"><i class="fas fa-calendar" style="color: #5d4037; width: 20px; margin-right: 10px;"></i><div><strong>Fecha:</strong> {{ formatearFecha(turnoSeleccionado.fecha) }}</div></div><div class="detail-row" class="btn"><i class="fas fa-clock" style="color: #5d4037; width: 20px; margin-right: 10px;"></i><div><strong>Hora:</strong> {{ formatearHora(turnoSeleccionado.hora) }}</div></div><div class="detail-row" class="btn"><i :class="getIconoEstado(turnoSeleccionado.estado)" :style="{ color: getColorEstado(turnoSeleccionado.estado), width: \'20px\', marginRight: \'10px\' }"></i><div><strong>Estado:</strong> <span :style="{ color: getColorEstado(turnoSeleccionado.estado), fontWeight: \'bold\', textTransform: \'capitalize\' }">{{ turnoSeleccionado.estado }}</span></div></div><div v-if="turnoSeleccionado.motivoCancelacion" class="detail-row" class="btn"><i class="fas fa-comment" style="color: #5d4037; width: 20px; margin-right: 10px;"></i><div><strong>Motivo Cancelación:</strong> {{ turnoSeleccionado.motivoCancelacion }}</div></div></div></div></div></main></div></div>'
});
